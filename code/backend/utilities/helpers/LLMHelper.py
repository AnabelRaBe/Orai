...
class LLMHelper:
    ...
    def get_streaming_llm(self, deployment_name: str, temperature: float, max_tokens: int):
        return AzureChatOpenAI(streaming=True, callbacks=[StreamingStdOutCallbackHandler], deployment_name=deployment_name, temperature=temperature, 
                               max_tokens=max_tokens, openai_api_version=openai.api_version)
